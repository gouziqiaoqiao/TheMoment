...
-(instancetype)initWithFrame:(CGRect)frame  totalTime:(NSInteger)totalTime lineWidth:(CGFloat)lineWidth  lineColor:(UIColor *)lineColor startBlock:(CountDownStartBlock)startBlock completeBlock:(CountDownCompleteBlock)completeBlock
{
    ...
    if (self) {
        ...
        countLabel.font = [UIFont fontWithName:@"Helvetica-Light" size:frame.size.width/4.5];
        ...
        dispatch_source_set_event_handler(_timer, ^{
            ...
            if(timeout<=0){ //倒计时结束，关闭
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    // 倒计时的计数
                    if (seconds > 60 ) {
                        NSInteger tempM, tempS;
                        tempM = seconds / 60;
                        tempS = seconds % 60;
                        if (tempM < 10) {
                            if (tempS < 10) {
                                self.countLabel.text = [NSString stringWithFormat:@"0%@:0%@",@(tempM), @(tempS)];
                            }
                            else {
                                self.countLabel.text = [NSString stringWithFormat:@"0%@:%@",@(tempM), @(tempS)];
                            }
                        }
                        else {
                            self.countLabel.text = [NSString stringWithFormat:@"%@:%@",@(tempM), @(tempS)];
                        }
                    }
                    else if (seconds <= 60) {
                        self.countLabel.text = [NSString stringWithFormat:@"%@",@(seconds).stringValue];
                    }
                    
                    // 拿到加速计并更新
                    WJGyroscope *gyroscope = [[WJGyroscope alloc] init];
                    
                    [gyroscope getPhoneAcceleration];
